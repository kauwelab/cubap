    ...
    with open(x, 'r') as f:
        ...
        for row in reader:
            if row[0] == 'Sample':
                ...
                for i in range(3,67):
                    codonDict[codons[i - 3]] += int(row[i])
    sample = x[x.find('/') + 1 :-11]
    d[sample] = []
    for i in range(3,67):
        d[sample].append(codonDict[codons[i - 3]])
    codonDict = codonDict.fromkeys(codonDict, 0)
    #pops[key[key['Sample'] == sample]['Subpopulation'].values[0]].append(total)

key = pd.read_csv(args.keyFile)
header = ['Sample','Subpopulation','Superpopulation'] + codons
with open('total_identical_codon_pairing_freqs.csv', 'w') as csv_file:
