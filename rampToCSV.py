...
def parseArgs():
    ...
    parser.add_argument("-e", "--efficiencies", help="Input FASTA Files of codon translation efficiencies as RSCU values",nargs='*',action="store", dest="efficiencies", required=True)
    parser.add_argument("-r", "--ramps", help="Input FASTA Files containing the ramp sequences",nargs='*',action="store", dest="ramps", required=True)
...
if __name__ =='__main__':
    ...
    for x in allInputFiles:
        with open(x, 'r') as f:
            ...
            if chart:
                to_csv_chart(sampleName, subpopulation, superpopulation, d, d_A)
            else:
                to_csv(sampleName, subpopulation, superpopulation, d, d_A)
